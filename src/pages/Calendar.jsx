...
import { todayKey, daysInMonth, firstWeekdayMonIndex, dateKeyFor, formatShortDate, daysBetween } from '../utils/date'
import { getNextOuting, sortOutingsAsc } from '../utils/outings'
...
export default function Calendar({ data, update }) {
  ...
  const outingsByDate = useMemo(() => {
    const map = new Map()
    for (const o of data.outings) map.set(o.date, o)
    return map
  }, [data.outings])

  const nextOuting = useMemo(() => getNextOuting(data.outings, today), [data.outings, today])
  const upcoming = useMemo(
    () => sortOutingsAsc(data.outings).filter((o) => o.date >= today),
    [data.outings, today]
  )
  ...
  function saveOuting(type) {
    update((prev) => {
      const others = prev.outings.filter((o) => o.date !== selectedDate)
      return { ...prev, outings: [...others, { id: `outing-${selectedDate}`, type, date: selectedDate }] }
    })
    setSelectedDate(null)
  }

  function deleteOuting() {
    update((prev) => ({ ...prev, outings: prev.outings.filter((o) => o.date !== selectedDate) }))
    setSelectedDate(null)
  }

  function removeOuting(date) {
    update((prev) => ({ ...prev, outings: prev.outings.filter((o) => o.date !== date) }))
  }
  ...
  return (
    <div className="page-shell">
        ...
        <div className="mt-6 rounded-2xl p-4" style={{ background: 'var(--surface)' }}>
          ...
          <div className="mt-1 flex items-end justify-between">
            <p className="text-3xl font-bold" style={{ color: 'var(--text)' }}>
              D-{daysBetween(today, nextOuting.date)}
            </p>
            <p className="text-sm" style={{ color: 'var(--text-2)' }}>
              {formatShortDate(nextOuting.date)}
            </p>
          </div>
        </div>
      ) : (
      ...
      <div className="mt-6">
        ...
        <div className="mt-3 grid grid-cols-7 gap-y-2 text-center">
          ...
          {cells.map((day, i) => {
            ...
            const outing = outingsByDate.get(dateKey)
            const isToday = dateKey === today
            const isNext = nextOuting?.date === dateKey
            return (
              <button
                key={day}
                  ...
                  style={{
                    ...
                    background: isNext
                      ...
                      : isToday
                        ? 'var(--surface)'
                        : 'transparent',
                    border: isToday && !isNext ? '1px solid var(--color-accent)' : 'none',
                    fontWeight: isToday || isNext ? 700 : 400,
                  }}
                >
                  {day}
                </div>
                <span className="text-xs leading-none">{outing ? typeEmoji(outing.type) : ' '}</span>
              </button>
            )
          })}
        </div>
          ...
          <div className="flex flex-col gap-2">
            {upcoming.map((o) => (
              ...
              >
                ...
                <button onClick={() => removeOuting(o.date)} style={{ color: 'var(--text-4)' }}>
                  <X size={16} />
                </button>
              </div>
            ))}
      ...
      <OutingModal
        ...
        date={selectedDate}
        existing={selectedDate ? outingsByDate.get(selectedDate) : null}
